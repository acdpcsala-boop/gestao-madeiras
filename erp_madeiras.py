...
credentials = {
    "usernames": {
        "alexandre": {
            "email": "luthieria@carreiroguitars.com",
            "name": "Alexandre Carreiro",
            "password": "$2b$12$eImiTXuWVxfM37uY4JANjO8e0.M2zW/4P/eE5pYjE3nKzD3N6A0Oq"  # Senha padrão: admin123
        }
    }
}
...
authenticator = stauth.Authenticate(
    credentials,
    "cookie_luthieria_erp",
    "chave_secreta_cookie_12345",
    cookie_expiry_days=30
)

# Renderiza o formulário de login
try:
    # Para versões mais recentes do streamlit-authenticator
    authenticator.login(fields={'Form name': 'Login - ERP Luthieria'})
except Exception:
    # Fallback para versões anteriores
    authenticator.login('Login - ERP Luthieria', 'main')

# Garante a verificação do estado na sessão
authentication_status = st.session_state.get("authentication_status")

if authentication_status is False:
    st.error("Usuário ou senha incorretos.")
elif authentication_status is None:
    st.info("🔒 Por favor, digite seu usuário e senha acima para acessar o sistema.")
    ...
    # Barra Lateral com Boas-Vindas e Botão Sair
    st.sidebar.title(f"👤 Olá, {st.session_state.get('name', 'Alexandre')}")
    try:
